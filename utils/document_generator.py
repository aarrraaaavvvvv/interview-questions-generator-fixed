...
# Base directory of the project (directory containing this utils/ folder)
_BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def _resolve_logo_path(relative_path: str) -> str:
    """Resolve a logo path relative to the project root, regardless of CWD."""
...
class PDFGenerator:
    ...
    def generate(self, qa_pairs: List[Dict]) -> bytes:
        if not WEASYPRINT_AVAILABLE:
            raise Exception("WeasyPrint not available. Install it with: pip install weasyprint")
...
class WordDocumentGenerator:
    ...
    def generate(self, qa_pairs: List[Dict], title: str, topic: str, partner_institute: str = "IIT Kanpur") -> bytes:
        ...
        # Resolve logo path relative to project root (not CWD)
        logo_relative = PARTNER_LOGOS.get(partner_institute, PARTNER_LOGOS["Default"])
        logo_path = _resolve_logo_path(logo_relative)
